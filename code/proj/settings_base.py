...
LOGGING = {
    ...
    "loggers": {
        "cidc-sys": {"handlers": ["default"], "level": "DEBUG", "propagate": True},
        "django.request": {
...
SYSTEM_SHORT_NAME = os.getenv('SYSTEM_SHORT_NAME', 'APP')
SYSTEM_LONG_NAME = os.getenv('SYSTEM_LONG_NAME', 'APP - App Name')
SYSTEM_OWNER = os.getenv('SYSTEM_OWNER', 'Company')

ACTUAL_ENV = os.getenv("APP_ENV", "dev")
    ...
    try:
        if ACTUAL_ENV == "dev":
            ...
            try:
                ...
                pass
            if os.getenv("DEBUG_PYCHARM", False):
        ...
        raise SystemError(
            "Settings file not found for {0}! Missing settings_{0}.py file".format(
                ACTUAL_ENV
            )
        )
else:
    raise SystemError("Environment not set! Export the APP_ENV env.")
