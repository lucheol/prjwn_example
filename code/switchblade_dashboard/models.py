    ...
    def _filter_by_str(cls, q, first_match_id=True, raise_when_not_found=True):
        ...
        if first_match_id:
            if qs.exists():
                ...
                raise ValueError(f'{cls._meta.object_name} {q} not found.')
            return None
...
class AuditLog(DashboardBaseModelNoUser):
    ...
    LEVEL_CHOICES = (
        (LEVEL_DEBUG, 'DEBUG'),
        (LEVEL_ACTION, _('ACTION')),
        (LEVEL_ACCESS, _('ACCESS')),
        (LEVEL_INFO, 'INFO'),
    ...
    level = models.CharField(verbose_name=_('Level'), max_length=10, choices=LEVEL_CHOICES, default=LEVEL_DEBUG)
    ...
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_('User'), on_delete=models.DO_NOTHING, blank=True, null=True)
