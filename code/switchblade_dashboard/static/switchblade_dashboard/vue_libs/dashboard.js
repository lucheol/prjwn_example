...
const vBarChart = {
    ...
    methods: {
        ...
        handleSeriesToEmulateNegativeZero: function (series, series_to_handle) {

            if (!series_to_handle || series_to_handle.length == 0) {
                return series;
            }

            series.forEach((series_data) => {
                if (!(series_to_handle.includes(series_data['name']))) {
                    return;
                }

                if ('stacked_sum' in series_data) {
                    return;
                }

                if (!('label' in series_data)) {
                    series_data['label'] = {'show': true}
                }

                if (!('tooltip' in series_data)) {
                    series_data['tooltip'] = {}
                }

                series_data['label']['formatter'] = function(d) {
                    let data;
                ...
                series_data['tooltip']['formatter'] = function(d) {
                    let icon = "<span style='display:inline-block;width:10px;height:10px;border-radius:50%;background-color:"+d.color+";margin-right:5px;'></span>";
                    let data;
                    ...
                    return d.seriesName + "<br>" + icon + d.name + ": " + data;
                }
            });
        ...
        handleSeriesToPreventNegativeValues: function (series, series_to_handle, series_to_emulate_negative_zero=[]) {
            if (!series_to_handle || series_to_handle.length == 0) {
                return series;
            }
            ...
            series.forEach((series_data) => {
                ...
                series_data['label']['formatter'] = function(d) {
                    ...
                    if (typeof d.data != "object") {
                        data = series_to_emulate_negative_zero.length > 0 && series_to_emulate_negative_zero.includes(series_data['name']) && d.data == -0.001? 0 : d.data;
                    } else {
                        data = d.data.value * -1;
                    }
...
const vCharts = {
    ...
    methods: {
        render: function() {
            ...
            if (this.chartActions && this.chartActions.length > 0) {
        ...
        refresh: async function (filters, render=false) {
            ...
            try {
                ...
                if (results.data.series_to_emulate_negative_zero) {
                    if (this.$children[0].handleSeriesToEmulateNegativeZero) {
                        results.data['series'] = this.$children[0].handleSeriesToEmulateNegativeZero(
                            results.data['series'],
                            results.data.series_to_emulate_negative_zero
                        );
                    }
                }

                if (results.data.series_to_prevent_negative_values) {
                    if (this.$children[0].handleSeriesToPreventNegativeValues) {
                        let series_to_emulate_negative_zero = results.data.series_to_emulate_negative_zero ? results.data.series_to_emulate_negative_zero : [];

                        results.data['series'] = this.$children[0].handleSeriesToPreventNegativeValues(
                            results.data['series'],
                            results.data.series_to_prevent_negative_values,
                            series_to_emulate_negative_zero
                        );
                    }
                }
        ...
        exportData: function () {
            ...
            axios.post('export/', {'id': this.id, 'page': this.position.page, 'position': this.position, 'filters': this.$parent.filters[this.position.page]}, { responseType: 'blob', ...getCsrfTokenHeader()}
            ).then((response) => {
        ...
        handleChartActions: function () {
            let self = this;

            if (this.chartActions.includes('legend_selection_as_filter')) {

                if (this.position == null ||
                    this.position == undefined ||
                    this.position.page == null ||
                    this.position.page == undefined) {
                    return;
                }

                this.chart.on('legendselectchanged', function(params) {
                    let page_filters = self.$parent.filters[self.$parent.currentPage];
                    let filters = Object.assign(page_filters, {'LEGEND_SELECTED': params.selected});

                    self.refresh(filters);
                });
            }
        },
    ...
    methods: {
        ...
        async showSeriesModal(clickEvent, id, title, position) {
            if(this.seriesModalRef && clickEvent && clickEvent.seriesIndex != null) {
                this.showModal = await this.seriesModalRef.render(clickEvent, id, title, position, this.filters[this.currentPage]);
            }
        },
