...
class AuditLogFilter(FilterSet):
    ...
    msg = CharFilter(label=_('Message contains'), field_name='msg', lookup_expr='icontains')
    trace = CharFilter(label=_('Traceback contains'), field_name='trace', lookup_expr='icontains')
    created_on = DateTimeRangeFilter(label=_('Created on'), widget=DateTimeRangeInput())

    level = ChoiceFilter(label=_('Level'), choices=AuditLog.LEVEL_CHOICES)
    if settings.USE_SWITCHBLADE_USER:
    ...
    class Meta:
        model = AuditLog
        fields = ['level', 'msg', 'trace', 'created_on', 'created_by']
