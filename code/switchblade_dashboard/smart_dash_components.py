...
class ECharts:
    ...
    def __init__(self, chart_id, title='', show_legend=True, extra_options={}, chart_actions=[],
                 charts_theme_name=None):
...
class BarChart(ECharts):
    ...
    def config(cls, series_columns, axis_column, orientation=BAR_CHART_ORIENTATION_VERTICAL, stacked_series={},
               ...
               rotate_label=0, series_to_prevent_negative_values=[], emulate_negative_zero=[]):
        return {
            'definitions': {
                'series_columns': series_columns,
                ...
                'series_to_prevent_negative_values': series_to_prevent_negative_values,
                'emulate_negative_zero': emulate_negative_zero
            }
        }

    @classmethod
    def _prevent_negative_values(cls, val):
        if not pd.isnull(val) and isinstance(val, int):
            if val < 0:
                return {"label": {"color": "#555"}, "value": val}

        return val

    @classmethod
    def _emulate_negative_zero(cls, data, series_to_apply, axis_column=None, axis_indexes=[]):
        if data.empty:
            return
        ...
        if axis_indexes:
            ...
            for series in series_to_apply:
                filtered_series = data[(data[axis_column].isin(axis_indexes)) & (data[series] == '')][series].replace(
                    to_replace='',
                    value=-0.001,
                    regex=True
                )

                data.update(filtered_series)
        else:
            for series in series_to_apply:
                filtered_series = data[data[series] == ''][series].replace(to_replace='', value=-0.001, regex=True)
                data.update(filtered_series)

    @classmethod
    def _format_series(cls, data, definitions={}, series_to_prevent_negative_values=[], emulate_negative_zero={}):
        ...
        if not data.empty and definitions:
            ...
            if stacked_series and stacked_sum:
                ...
                # Convert Negative values to positive in to stack sum
                if series_to_prevent_negative_values:
                    data_as_numeric[series_to_prevent_negative_values] = \
                        data_as_numeric[series_to_prevent_negative_values].abs()
            ...
            # Convert negative values to custom objects
            if series_to_prevent_negative_values:
                for series in series_to_prevent_negative_values:
                    data[series] = data[series].apply(cls._prevent_negative_values)

            # Convert empty values to "-0.001" due to emulate "negative zero"
            if emulate_negative_zero:
                series_to_emulate_negative_zero = emulate_negative_zero.get('series', [])
                axis_column = definitions.get('axis_column', None)
                axis_indexes_to_apply = emulate_negative_zero.get('axis_indexes_to_apply', [])

                cls._emulate_negative_zero(data, series_to_emulate_negative_zero, axis_column, axis_indexes_to_apply)
    ...
    def get_data(cls, data={}, legend={}):
        ...
        series_columns = definitions.get('series_columns', [])
        series_to_prevent_negative_values = definitions.get('series_to_prevent_negative_values', [])
        emulate_negative_zero = definitions.get('emulate_negative_zero', {})

        if definitions:
            df = cls.sort_data(df, series_columns) if series_columns else df

        percentage_mode_series = cls.format_percentage_mode_series(df, series_columns) if stacked_series and percentage_mode else {}
        series = cls._format_series(df, definitions, series_to_prevent_negative_values, emulate_negative_zero)
        ...
        series_to_prevent_negative_values = {
            'series_to_prevent_negative_values': series_to_prevent_negative_values
        } if series_to_prevent_negative_values else {}

        if emulate_negative_zero:
            series_to_emulate_negative_zero = emulate_negative_zero.get('series', [])

            series_to_emulate_negative_zero = {
                'series_to_emulate_negative_zero': series_to_emulate_negative_zero
            } if series_to_emulate_negative_zero else {}
        else:
            series_to_emulate_negative_zero = {}
